package com.hzmy.zm.mysettingdemo.ui.widget;
        ...
        dialog.setPositiveButton(R.string.confirm, new DialogInterface.OnClickListener() {
            public void onClick(DialogInterface dialog, int which) {
                ...
                if (TextUtils.isEmpty(old)) {
                    ToastUtils.shortToast(context, R.string.original_password);
                    return;
                }
                if (TextUtils.isEmpty(new1)) {
                    ToastUtils.shortToast(context, R.string.new_password_not_null);
                    return;
                }
                if (TextUtils.isEmpty(new2)) {
                    ToastUtils.shortToast(context, R.string.confirm_password_not_null);
                    return;
                }
                if (!cachePassword.equals(old)) {
                    ToastUtils.shortToast(context, R.string.original_password_mistake);
                    return;
                }
                if (!new1.equals(new2)) {
                    ToastUtils.shortToast(context, R.string.passwords_do_not_match);
                    return;
                }
